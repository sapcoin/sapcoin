// Copyright (c) 2009-2010 Satoshi Nakamoto2012 The Bitcoin developers
...
//uint256 hashGenesisBlock("0x000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f"); // original
uint256 hashGenesisBlock("0x00000000040555f579d87904a93926421849e59d7de4099507bb38d8bb77b5de");
//static CBigNum bnProofOfWorkLimit(~uint256(0) >> 32); //original
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); //litecoin started value
...
const string strMessageMagic = "SAPCoin Signed Message:\n";
...
static const int64 nTargetTimespan = 7 * 24 * 60 * 60;  // one week (originally: 14 two weeks)
static const int64 nTargetSpacing = 3 * 60; // just 3 min.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Go back by what we want to be 14 days worth of blocks (SAPCoin changed from 14 to 7)
    const CBlockIndex* pindexFirst = pindexLast;
    for (int i = 0; pindexFirst && i < nInterval-1; i++)
        pindexFirst = pindexFirst->pprev;
    assert(pindexFirst);

    // Limit adjustment step (SAPCoin changed from 4 to 3)
...
bool VerifyDB() {
	...
	int nCheckDepth = GetArg("-checkblocks", 960); // 288 is for 10min period: 6 (blocks/hr) x 24 x 2 = 288
    if (nCheckDepth == 0)
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
		...
		//const char* pszTimestamp = "The Times 03/Jan/2009 Chancellor on brink of second bailout for banks"; // original
		const char* pszTimestamp = "CoinDesk, On the Verge of Merging Blockchain Code from IBM, Digital Asset";
        ...
        block.nVersion = 1;
		block.nTime = 1458941103; // orig 1231006505
		block.nBits = 0x1d00ffff; // orig 0x1d00ffff
		block.nNonce = 948622141; // orig 2083236893 
		/*
		printf("Calling Generate Genesis Block...\n");
		generate_genesis_block();
		*/
        if (fTestNet)
        {
			block.nTime = 1458941103; // from 1296688602 to 1463799600 (21/05/2016 00:00:00)
			block.nNonce = 948622141;
        }
        ...
        //assert(block.hashMerkleRoot == uint256("0x4a5e1e4baab89f3a32518a88c31bc87f618f76673e2cc77ab2127b7afdeda33b")); // original
		assert(block.hashMerkleRoot == uint256("0x07c6c31c1b84fd798b24a8b0a2fc02d04cae31429d7c40ac4ea6a2bd86b69c1b"));
        block.print();
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SAPCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SAPCoinMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("SAPCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sapcoin-miner");
    ...
    try { loop {
        ...
        printf("Running SAPCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SAPCoinMiner terminated\n");
        throw;
    }
}
...
void generate_genesis_block()
{
	const char* pszTimestamp = "On the Verge of Merging Blockchain Code from IBM, Digital Asset - CoinDesk";
	...
	block.nNonce = 100000000; //orig 2083236893 
