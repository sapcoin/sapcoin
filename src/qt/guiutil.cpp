...
{
    // Convert sapcoin:// to sapcoin:
    //
    //    Cannot handle this later, because sapcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("sapcoin://"))
    {
        uri.replace(0, 10, "sapcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "SAPCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for SAPCoin.lnk
...
{
    return GetAutostartDir() / "sapcoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a sapcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=SAPCoin\n";
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("SAPCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  sapcoin-qt [" + tr("command-line options") + "]                     " + "\n";
