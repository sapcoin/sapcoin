...
{
    QString ret = QString("sapcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non SAP unit input in lnReqAmount, we generate the URI with SAP as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::SAP, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
